...
SizeMem = sizeof(*counters)*3;

shmid = shmget(key, 3*sizeof(int), IPC_CREAT);

counters = (int *)shmat(shmid, 0, 0);

counters[0] = rockCount;

counters[1] = paperCount;

counters[2] = scissorsCount;
...
shmid = shmget(key, 3*sizeof(int), IPC_EXCL);

counters = shmat(shmid, 0, SHM_RDONLY);
